...
var gooSlider = function (parent, id) {
  ...
  var dripDist = 90; // distance between each "drip"
  var dripsAmount = Math.floor(size.x / dripDist) + 3; // amount of drips that will go across screen
  ...
  // let s = Math.random();
  // const n = 3;
  // s = Math.floor(s * Math.pow(10, n)) / Math.pow(10, n);
  // console.log(s);
  noise.seed(0.178); //固定値で行く
...
const target = document.getElementById("news"); // 対象のエレメント
...
// 交差した際の処理を記載
let func = function (entries, observer) {
  entries.forEach(function (entry) {
    // 交差している場合はtrue
    if (entry.isIntersecting) {
      let slider = new gooSlider("#wave");
      function start() {
        slider.wave("hsla(" + 100 + ",82%,64%,1)");
      }
      start();
      // // 処理実行
      // // 処理完了後、監視を止めたい場合
...
var observer = new IntersectionObserver(func, option); // callback, optionを設定
